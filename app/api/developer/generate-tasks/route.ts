...
import { generateTasksFromDeveloperDocumentationFromGeminiAI } from "@/lib/ai-client";
import {
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Invalid request format. Expected JSON with a textContent field.",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Text content is required and must be a non-empty string containing the documentation to analyze.",
        },
        { status: 400 }
      );
    }
    ...
    // Generate tasks
    const tasks = await generateTasksFromDeveloperDocumentationFromGeminiAI(
      textContent
    );
