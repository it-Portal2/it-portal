...
- **Payments**: clients pay either **online via PayU** (Direct tab) or **manually** (receipt upload). PayU is hosted-checkout, **INR only**, and **production-only** (endpoint hardcoded to `secure.payu.in` in [lib/payu.ts](lib/payu.ts) — the test/prod selector was removed). Credentials (key/salt/clientId/clientSecret/appBaseUrl) live in Firestore `paymentConfig/payu` (server-only — never `.env`), configured in Admin → Settings → Payment. SHA-512 hashing is server-side; flow is `POST /api/payu/initiate` (verifies token, recomputes amount from the project, applies any coupon) → PayU → `POST /api/payu/callback` (verifies response hash; idempotent `finalizePayuTransaction` activates the project to `in-progress` once). **Coupons**: admins create them in Admin → Settings → **Create Coupon** (`coupons` collection, doc id = uppercased code, flat ₹ or % discount, active toggle); clients apply a code on the Direct tab. The discount is computed **server-side** in initiate via the shared `applyCoupon()` (clamped to a ₹1 floor) — the client preview (`validateCouponAction`) is display-only. Collections: `paymentConfig/payu`, `payuTransactions` (doc id = txnid, idempotency; stores `couponCode`/`originalAmount`), `coupons`, `internationalBankAccounts` (admin-managed per-country accounts with dynamic `fields[]`), plus existing `ClientPaymentDetails` (unified log — PayU successes mirrored here as `verified`) and `PaymentDetails` (UPI/PayPal/bank/crypto config, now with an `active` map). Services in [lib/firebase/payments.ts](lib/firebase/payments.ts); actions in [app/actions/payment-actions.ts](app/actions/payment-actions.ts) (admin writes gated via the `firebaseToken` cookie). PayU `surl`/`furl` come from `appBaseUrl` (must be a public HTTPS origin).
...
- **Token cookie is httpOnly (server-set):** the `firebaseToken` cookie is now set server-side as httpOnly via `POST /api/setAuthCookie` (and cleared via `POST /api/clearCookie`), readable only by the middleware — **not** by client JS. Client code calls `setAuthCookie(token)`/`clearAuthCookie()` from [lib/auth-cookie.ts](lib/auth-cookie.ts); never write `document.cookie` for the token. (Historically it was non-httpOnly "for a Vite cross-origin handoff" — but no such handoff exists in this repo; if one is added, mint an on-demand token and pass it via the middleware's inbound `?token=` param, which now also sets the cookie httpOnly.)
...
- **CRITICAL security fix (done):** `POST /api/setCustomClaims` previously set any `{uid, role}` with **no auth** → anyone could self-assign `role:"admin"` (privilege escalation). Now it verifies the caller's Firebase ID token and authorizes: admins/subadmins may set any role for any uid; everyone else may only self-assign the base `client` role (the signup path). The client helper sends `Authorization: Bearer <idToken>`. GET is also auth-gated (self or admin).
- **Track B — auth-cookie httpOnly hardening (done):** `firebaseToken` is now a server-set **httpOnly** cookie via [app/api/setAuthCookie/route.ts](app/api/setAuthCookie/route.ts) (verifies the token before setting). All client `document.cookie` writes replaced with [lib/auth-cookie.ts](lib/auth-cookie.ts) helpers; `useAuth` (signup/login/google/logout) and `userStore.onIdTokenChanged` updated; middleware inbound `?token=` cookie is httpOnly too. Investigation confirmed **no Vite token handoff exists in this repo** (no `NEXT_PUBLIC_VITE` usage, no JS cookie reads), so httpOnly is safe. Dead `lib/firebase/authService.ts` (misleading httpOnly-via-JS) deleted.
- **Client loading-shell fix (done):** the client portal renders `<Layout>` inside each page (no `app/client/layout.tsx`, because `create-project` is intentionally shell-less), so its `loading.tsx` skeletons appeared full-page without the sidebar. Fixed by wrapping each client loading skeleton in [components/ui-custom/client-loading-shell.tsx](components/ui-custom/client-loading-shell.tsx) (renders the `Layout` shell), so the sidebar/header stay during navigation — matching admin/developer (which get the shell from their `layout.tsx`). `create-project/loading.tsx` left shell-less to match its page. Note: the proper long-term fix is route-group layouts (`app/client/(shell)/layout.tsx`) so the sidebar persists without remounting — deferred (larger refactor).
- **Still open (best-practice, not done):** plaintext passwords stored in Firestore (`createClient`), module-level `initializeAuth()` side effect at import, duplicate Firestore reads per login, SSR hydration of persisted auth state.
